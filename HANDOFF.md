...
- KB 서버(`ppa.kbinsure.co.kr`)는 **한국 IP(가정·회사 ISP)에서는 로그인 없이 접속**되나,
  **GitHub Actions(해외 데이터센터 IP)는 타임아웃으로 막힘** → **사실상 로컬 실행 필요**.
  (인증 문제 아님 — 한국 금융사 흔한 해외/데이터센터 IP 차단 추정. "사내망 전용"은 아님: 집에서도 됨.)
  클라우드로 강행하려면 한국 리전 VM(AWS 서울 `ap-northeast-2`) 또는 한국 IP self-hosted runner 시도 가능(미검증).
